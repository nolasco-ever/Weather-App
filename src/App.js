...
{city: 'Zuni Pueblo', state: 'New Mexico'}
  ...
  function inputTextHandler(e){
    ...
    //count how many queries have been added to suggest state (only allow up to 25 location suggestions)
    var count = 0;

    // Add to suggestion array
    for (let i = 0; i < cities.length; i++){
      if((cities[i].city.toUpperCase()+', '+cities[i].state.toUpperCase()).startsWith(e.target.value.toUpperCase())){
        if (count < 25){
          const thisCity = cities[i].city;
          const thisState = cities[i].state;
  
          // fetch weather information from API
          // const thisDataToday = await fetch(
          //   'https://api.openweathermap.org/data/2.5/weather?q='+thisCity+','+thisState+'&appid='+API_key
          // ).then((res) => res.json())
          // .then((thisDataToday) => thisDataToday);
  
          // const thisLong = thisDataToday.coord.lon;
          // const thisLat = thisDataToday.coord.lat;
  
          // const thisData = await fetch(
          //   'https://api.openweathermap.org/data/2.5/onecall?lat='+thisLat+'&lon='+thisLong+'&units=imperial&appid='+API_key
          // ).then((res) => res.json())
          // .then((thisData) => thisData);
  
          // const thisTemp = thisData.hourly[0].feels_like;
          // const thisCloudType = thisData.hourly[0].weather[0].main;
          // var thisWeatherIcon;
  
          // switch(thisCloudType){
          //   case "Thunderstorm":
          //         thisWeatherIcon = thunderstormDay;
          //         break;
          //     case "Drizzle":
          //         thisWeatherIcon = drizzleDay;
          //         break;
          //     case "Rain":
          //         thisWeatherIcon = rainDay;
          //         break;
          //     case "Snow":
          //         thisWeatherIcon = snowDay;
          //         break;
          //     case "Clear":
          //         thisWeatherIcon = clearDay;
          //         break;
          //     case "Clouds":
          //         thisWeatherIcon = cloudyDay;
          //         break;
          //     default:
          //         thisWeatherIcon = fogDay;
          // }
  ...
  async function getWeatherData(e){
    ...
    if(location.includes(',')){
      ...
      if ((Date.now()/1000) > data.current.sunrise && (Date.now()/1000) < data.current.sunset){
        setIsDay(true);
      }
      else{
        setIsDay(false);
      ...
      var currentTemp = data.hourly[0].temp;
